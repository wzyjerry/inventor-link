...
'''
inputs是一个形如(batch_size, seq_len, word_size)的张量；(32, 500, 50)
函数返回一个形如(batch_size, seq_len, position_size)的位置张量。(32, 500, ps)
'''
def Position_Embedding(inputs, position_size):
    ...
    position_j = tf.expand_dims(position_j, 0) # (1, ps/2), 1/(10000^(2i/ps))
    position_i = tf.range(tf.cast(seq_len, tf.float32), dtype=tf.float32)
    position_i = tf.expand_dims(position_i, 1) # (500, 1), p
    position_ij = tf.matmul(position_i, position_j) # (500, ps/2)
...
'''
inputs是一个二阶以上的张量，代表输入序列，比如形如(batch_size, seq_len, input_size)的张量；(32, 500, 50)
seq_len是一个形如(batch_size,)的张量，代表每个序列的实际长度，多出部分都被忽略；(32,)
